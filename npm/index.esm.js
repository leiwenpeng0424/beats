import { colors, json, module_, file, ms, parser } from '@nfts/nodeutils';
...
const depsInfo = () => {
  ...
  const depInfo = coreDeps.map((dep) => {
    ...
    const pkgJson = json.readJSONSync(
...
const cli = async (args) => {
  const [, ..._args] = args;
  const pkgJson = json.readJSONSync(packageJsonFilePath);
