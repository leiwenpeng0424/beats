...
import { Terminal, colors, file, ms, m, json, parser } from '@nfts/nodeutils';
import nodePath from 'node:path';
import ts, { readJsonConfigFile, sys, parseJsonSourceFileConfigFileContent, createIncrementalCompilerHost, createIncrementalProgram } from 'typescript';
...
function createCompilerProgram(tsConfigCompilerOptions, tsconfig) {
  ...
  if (config) {
    return createIncrementalProgram({
      host,
      options: __spreadProps$1(__spreadValues$2({}, config.options), {
        noEmit: false
      }),
...
function dtsRollup(_0) {
  return __async$3(this, arguments, function* ({
    ...
    const content = yield nodeFsPromise.readFile(mainEntry);
    if (content.toString() === "") {
      return;
    }
...
const bundle = (_0) => __async$2(void 0, [_0], function* ({
  options,
  config,
  pkgJson
}) {
  ...
  try {
    for (var iter = __forAwait$1(options), more, temp, error; more = !(temp = yield iter.next()).done; more = false) {
      ...
      if (output) {
        ...
        for (const output_ of output) {
          bundles.push(() => __async$2(void 0, null, function* () {
            ...
            log.info(`${message}`);
            return __spreadProps(__spreadValues$1({}, output2), {
              duration
            });
          }));
        }
...
const watch_ = (_0, _1) => __async$2(void 0, [_0, _1], function* (options, {
  config,
  pkgJson
}) {
    ...
    if (config.bundle) {
      bundles = config.bundle.reduce((options, bundle2) => {
        ...
        const option = __spreadValues$1({
          ...
          output: [
            __spreadProps(__spreadValues$1({}, otherProps), {
              sourcemap,
              paths: pathReWrite
            })
          ]
...
const esFormatFileRegexp = (name) => /((.+)\.m?js)|((.+)\.esm?\.js)/g.test(name);
...
function tryReadConfig(_0) {
  return __async$1(this, arguments, function* ({
    ...
    if (!configPath) {
      try {
        for (var iter = __forAwait(Configs), more, temp, error; more = !(temp = yield iter.next()).done; more = false) {
          const configFile = temp.value;
          try {
            const configFilePath = nodePath.join(_cwd, configFile);
            yield nodeFsPromise.access(configFilePath);
